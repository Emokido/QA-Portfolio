...
| 결과 | 확인한 범위와 의미 |
|---|---|
| **핵심 우선순위 테스트(P0) 49건 <br>  45 Pass·4 Fail** | 리뷰·댓글의 CRUD, 권한, 입력 경계, 조회·정렬 등을 우선 검증했습니다. 테스트 구성 오탐 2건은 실제 서버 응답과 교정 테스트를 근거로 Pass로 재분류했습니다. 45 Pass·4 Fail은 케이스별 최신 유효 판정을 집계한 값이며, 49건 전체를 한 번에 재실행한 결과는 아닙니다. |
