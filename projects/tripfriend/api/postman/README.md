...
사전조건은 H2 백엔드 8080과 격리된 임시 Redis다. 브라우저 흐름을 함께 확인할 때만 프런트 3000이 필요하다. 기존 Windows Redis 6379를 사용하지 않고 승인된 QA 실행 설정의 임시 Redis 6380을 사용한다. 실제 계정 대신 `BaseInitData`의 로컬 테스트 회원을 사용한다.

1. Environment의 `testUsername`과 `testPassword`에 안내받은 로컬 테스트 값만 입력한다. 공개 파일이나 스크린샷에는 남기지 않는다.
...
7. `05 Defect Verification / TF-S8-POSTMAN-007`에서 평점 0이 HTTP 400·code `400-2`인지 확인한다. 실제 `400-1`이면 TF-BUG-003 재현 Fail 후보다.
...
- TF-S8-POSTMAN-006은 HTTP 401·빈 본문인데도 최초의 넓은 4xx assertion 때문에 통과한 테스트 오탐이었다. 사용자 재현과 승인된 제한 진단에서 기존 TF-BUG-006 경로로 판정했다.
...
- 서버 미기동·승인 밖 외부 의존성 등으로 실행할 수 없으면 `Blocked` 후보이다.
