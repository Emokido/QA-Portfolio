...
- 인증 단위는 설치된 로컬 Redis를 디스크 저장 없이 임시 사용한다. 최초 6379 계획은 기존 자동 시작 Redis 서비스의 포트 점유·비격리·디스크 저장 위험 때문에 실행 전 중단했으며, 당시에는 QA 실행 전용 포트 6380 변경 권한 대기 상태였다. 이후 격리 기동 완료 기록은 API·프런트 실행 보고서를 사용한다.
